  ...
  function setup(hint) {
    ...
    hint.simpleTooltipHandler = function() {
      ...
      if (shift.length < 5) {
        ...
        var noPixelsAtRight =
          window.innerWidth - (dimensions.left + dimensions.width + 20) <
          plugin.maxWidth
  ...
  window.addEventListener('DOMContentLoaded', function() {
    for (var type in containers) {
      var element = document.body.appendChild(
        document.createElement(containers[type])
      )
      ...
      for (var i = 0, all = this.length || 1; i < all; i++) {
        ...
        if (options.destruct) {
          ...
          try {
            ...
            options.shift =
              plugin.shifts[
                plugin.shifts.indexOf(
                  options.shift || element.getAttribute(plugin.attrs.shift)
                )
              ]
          } catch (e) {
            continue
